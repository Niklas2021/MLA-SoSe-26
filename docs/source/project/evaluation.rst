Evaluation
==========

.. Sammelseite. Inhalt:
   - Aufbau erklaeren: erst die Methodik (inklusive eines Messartefakts, das man
     kennen muss, um die Zahlen zu lesen), dann GB10, dann RTX 3070, dann der
     Vergleich der beiden Karten.
   - Ein Absatz zur Datenbasis mit Verweis auf die Ordner:
     result_dgx_v1 (erster GB10-Sweep), results_dgx_v2 (zweiter GB10-Durchlauf,
     alles in einer Session), result_3070 (erster 3070-Sweep, mit Einschraenkung),
     results_3070_v2 (zweiter 3070-Durchlauf).

.. toctree::
   :maxdepth: 1

   evaluation/methodik
   evaluation/gb10
   evaluation/rtx3070
   evaluation/vergleich
